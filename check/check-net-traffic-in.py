...
def check_net_traffic_in(threshold=5242880000):
    cmd = "cat /proc/net/dev|awk '{print $2}'|grep -E '^[0-9]' > /tmp/$USER/recv_s;sleep 1;"
    cmd += "cat /proc/net/dev|awk '{print $2}'|grep -E '^[0-9]' > /tmp/$USER/recv_e;"
    cmd += "pr -m -t /tmp/$USER/recv_s /tmp/$USER/recv_e|awk '{print $1,$2}'|awk '{if($2-$1 >100)print $2-$1}'"
    cmd += "|sort -nr|head -n1"
    ...
    last_msg = "获取网络进流量占比值: %s, 合理情况应当小于阈值: %s" % (current, threshold)
    if float(current) > float(threshold):
        msg = "获取网络进流量占比值: %s, 大于合理阈值: %s" % (current, threshold)
        logging.error(msg)
        return 1
