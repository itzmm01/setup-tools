    ...
    for mariadb in ${mariadb_list[@]}; do
        ...
        if [ "$mode" == "MS" ]; then
            for pod in $(kubectl -nsso get pods -lssm.infra.tce.io/role=slave | grep -v NAME | grep $mariadb | grep mariadb | awk '{print $1}'); do
                current=$(kubectl -nsso exec -i $pod -c xenon -- mysql -uroot -p123456 -e "show slave status \G;" | grep Last_Error|awk -F 'Last_Error:' '{print $2}')
                if [ ! "$current" == ' ' ]; then
                  print_log "ERROR" "Last_Error: ${mariadb} ${pod} ${current}"
                fi
            done
        fi
    done
