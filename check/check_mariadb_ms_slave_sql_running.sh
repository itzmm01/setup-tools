    ...
    for mariadb in ${mariadb_list[@]}; do
        ...
        if [ "$mode" == "MS" ]; then
            for pod in $(kubectl -nsso get pods -lssm.infra.tce.io/role=slave | grep -v NAME | grep $mariadb | grep mariadb | awk '{print $1}'); do
                current=$(kubectl -nsso exec -i $pod -c xenon -- mysql -uroot -p123456 -e "show slave status \G;" | grep Slave_SQL_Running | grep No | wc -l)
                if [ $current -eq 1 ]; then
                    let error_num=error_num+1
                    print_log "ERROR" "${mariadb} ${pod}集群Slave_SQL_Running 状态异常"
                else
                    print_log "INFO" "${mariadb} ${pod}集群Slave_SQL_Running 状态正常"
                fi
            done
        else
...
function main() {
    ...
    if [ $error_num -eq 0 ]; then
        print_log "INFO" "所有主从模式下的mariadb集群Slave_SQL_Running 状态正常"
        exit 0
    else
        print_log "ERROR" "至少有1个主从模式下的mariadb集群Slave_SQL_Running 状态异常，请进行检查分析"
        exit 1
    fi
}
