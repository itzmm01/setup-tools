...
# Name: check-k8s-ns.sh
# Description: a script to check if given namespaces already
...
print_usage() {
    print_log "INFO" "Desc: Check if given namespaces exist in current k8s platform"
...
function main()
{
    ...
    if command -v kubectl > /dev/null 2>&1 ; then
        ...
        for ns in "$@"; do
            ...
            if [ "${hasNs}" != "" ]; then
                print_log "INFO" "Namespace ${ns} already exists"
            else
                print_log "ERROR" "Namespace ${ns} does not exist"
                isAllOk=1
            fi
        done
        if [ "${isAllOk}" == "1" ] ; then
