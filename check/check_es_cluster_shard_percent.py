...
def check_es_cluster_shard_percent(app_name=None):
    ...
    for es in es_list:
        ...
        for pod in pod_list:
            cmd = "kubectl -nsso exec -i %s -- sh -c '(curl -sS %s http://localhost:9200/_cluster/health?pretty " \
                  "--connect-timeout 5|grep active_shards_percent_as_number)'" % (pod, url)
            ret_msg = os.popen(cmd).read().strip()
            if not ret_msg:
                msg = "%s 集群状态shard失败：%s" % (pod, ret_msg)
                logging.error(msg)
                normal += 1
                continue

            status = ret_msg.strip()
            if "100" not in status:
                msg = "%s 集群状态shard完整数不是100 %%：%s" % (pod, status)
                logging.error(msg)
                normal += 1
                continue

            msg = "%s shard完整数100 %%：%s" % (pod, status)
            logging.info(msg)
    return normal
