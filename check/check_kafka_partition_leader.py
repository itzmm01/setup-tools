...
parser.add_argument('--threshold', help='kafka的没有活跃leader的阈值', type=int, default=0)
...
def kafka_partition_leader(app_name=None, threshold=0):
    ...
    for kafka in kafka_list:
        ...
        for pod in pod_list:
            cmd = "kubectl -nsso exec -i svc/kafka-%s -- sh -c '(curl -sS http://localhost:%s/metrics --connect-timeout 5)'" \
                  "|grep kafka_controller_kafkacontroller_offlinepartitionscount|egrep -v \"__consumer_offsets|TYPE|HELP\"""" % (
                      kafka, metric_port)
            logging.info(cmd)
            ret_msg = os.popen(cmd).readlines()
            if not ret_msg:
                msg = "%s 集群获取partition的leader失败：%s" % (pod, ret_msg)
                logging.error(msg)
                return 1
            ...
            for line in ret_msg:
                ...
                partition = line.split()[0].lstrip().rstrip()
                count = line.split()[1].lstrip().rstrip()
                if float(count) != float(threshold):
                    msg = "%s 的topic %s 没有活跃的leader: %s " % (pod, partition,count)
                    logging.error(msg)
                    return 1

        msg = "%s 的所有topic的partition都含有活跃的leader" % (kafka)
        logging.info(msg)


if __name__ == '__main__':
    exit(kafka_partition_leader(app_name=args.app_name, threshold=args.threshold))
