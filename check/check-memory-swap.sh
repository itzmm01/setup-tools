...
function check_threshold()
{
    ...
    swapOn=$(free -g | grep -i 'swap' | awk '{if ($2==0) pass;else if (int($3/$2)>$threshold) print $N}' | wc -l)
    if [[ $swapOn -eq 0 ]];then
        print_log "INFO" "[Ok] Current: $swapOn, required: $threshold."
        return 0
    else
        print_log "ERROR" "[Nok] Current: $swapOn, required: $threshold."
        return 1
    fi
}
